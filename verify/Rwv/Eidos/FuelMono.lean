/-
Fuel monotonicity for the Synolon semantics, expression half: every
fueled function in the value layer (Rwv.Eidos.Value), the decoder
(Rwv.Eidos.Decode), and the pure evaluator (Rwv.Eidos.Eval) preserves
successful results under more fuel — `fuel ≤ fuel' → f fuel … = .ok v
→ f fuel' … = .ok v`, with the SAME value. The machine layer
(Rwv.Synolon.Machine) is Rwv.Synolon.FuelMono, compositionally over
these lemmas; there, "∃ fuel, the run succeeds" becomes a canonical
fuel-independent semantics (`Proc.run_fuel_stable`,
`Proc.run_fuel_deterministic`). The `Except` plumbing, the traversal
combinators, and the extern-environment extension relation `EExt` the
machine half threads live in the nested namespace `FuelMono`.
...
- The machine layer (Rwv.Synolon.FuelMono) compositionally, monotone
  in the evaluation fuel and the goto fuel separately (`execBlock`'s
  terminator runner by induction on the goto fuel, which every
  recursive position decrements).
-/
...
theorem bind_ok {α β : Type} {x : Except String α} {f : α → Except String β}
...
successful result. -/
theorem mapM_mono {α β : Type} {g g' : α → Except String β}
...
theorem foldlM_mono {σ α : Type} {g g' : σ → α → Except String σ}
...
theorem forIn_mono {α σ : Type} {g g' : α → σ → Except String (ForInStep σ)}
...
def EExt (E E' : Rwv.Hyle.Sem.EEnv) : Prop :=
  ∀ s gs f, E s gs = some f → E' s gs = some f

theorem eext_refl (E : Rwv.Hyle.Sem.EEnv) : EExt E E := fun _ _ _ h => h

theorem eext_empty (E : Rwv.Hyle.Sem.EEnv) : EExt Rwv.Hyle.Sem.eEmpty E :=
...
at once): the form the machine layer (Rwv.Synolon.FuelMono) threads. -/
theorem eval_trans {Δ : DEnv} {defns : HashMap Int Defn}
