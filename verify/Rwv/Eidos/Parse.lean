/-
Parser for the Eidos concrete syntax (.eir; doc/eidos.md §9): the
lexer, the combinators, and the type, expression, definition, and
datatype productions, plus the expression elaborator — a transcription
of the reference implementation (rewire-frontend ReWire.Eidos.Parse),
in the same tokenizer + recursive descent style as Rwv.Hyle.Parse. The
Synolon extension (.syn; doc/synolon.md §9: processes, programs, and
the entry point `parseSyn`) is Rwv.Synolon.Parse, over this module.
