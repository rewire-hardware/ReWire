...
doc/synolon.md §4, §6), expression half: the whole-program pre-ToHyle
check, and the hypothesis set for the Eidos↔Hyle simulation
metatheorem. This module holds the rendering helpers, the builtin
signature table, and the datatype, definition, expression, and
global-uniqueness rules; the per-process machine rules,
pure-acyclicity, and the whole-program entry point
`Program.checkMachine` are Rwv.Synolon.Check, over this module.
...
equations), checked at every `Prim` occurrence; and the value-binder
half of a rule of doc/synolon.md §4 that the Haskell linter
(ReWire.Synolon.Lint) also enforces:

* **representability** — every value binder has a fixed bit width
  (`DEnv.sizeOf`, shared with the translation's sizing in
  Rwv.Eidos.Value; the reference's is ReWire.Synolon.Repr, shared
  between its lint and its fold) — so `Integer` = 128 bits, recursive
  datatypes are rejected, open widths are rejected. (Block parameters,
  state cells, process ports, and halt answers are sized in
  Rwv.Synolon.Check, where the other such rule, pure-acyclicity,
  lives too.)
...
expression-level `case` does, and terminator-case literal alternatives
are distinctness-checked, like expression-level ones (both in
Rwv.Synolon.Check); and `checkTop`'s
...
def mkEnv (datas : List DataDefn) (defns : List Defn) : Env where
  Δ     := DEnv.ofDatas datas
  cons  := HashMap.ofList (datas.flatMap fun d => d.cons.map fun c => (c.name, (d.name, c.sig)))
  scope := HashMap.ofList (defns.map fun d => (d.name.uniq, d.name))
  tvs   := ∅
  joins := ∅
  tail  := ∅
...
def uniqSites (datas : List DataDefn) (defns : List Defn) : Except String (List Site) := do
