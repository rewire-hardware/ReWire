...
\begin{verbatim}
--
-- should only be called on exprs whose types are of form
--
--   StateT^n (ReactT (StateT^m Id)) a where n > 0 or m > 0.
--
-- If n > 0, it'll spit out an expr of type
--
--   StateT^(n-1) (ReactT (StateT^m Id)) (a,s)
--
-- where s is the type of the state for the first StateT.
--
-- If n == 0 and m > 0, it'll spit out an expr of type
--
--   ReactT (StateT^(m-1) Id) (a,s)
--
-- where s is the type of the state for the first StateT.
--
-- The "statename" should be pre-loaded with the appropriate
-- name, which will have been generated when we entered a
-- function body.
--
tr :: ReExp -> M ReExp
tr "f e1..en" = do
  s  <- statename
  f' <- schedule f for tr'ing
  if s exists then
    return "f' e1 .. en s"
  else
    return "f' e1 .. en"

tr "let x = e in e_r end" = do
  e'   <- statelessly (tr e)
  e_r' <- assuming x has type of e' $ tr e_r
  return "let x = e' in e_r' end"

tr "case e of inl x -> ex ; inr y -> ey" = do
  -- e has type (t,t')
  e'  <- statelessly (tr e)
  ex' <- assuming x has type t  $ tr ex
  ey' <- assuming y has type t' $ tr ey
...
tr "bind x <- e1 in e2" = do
  -- e1 has type M t for some monad M
  e1' <- tr e1
  e2' <- assuming x has type t $ tr e2
  s   <- statename
  p   <- gensym
  return "bind p <- e1' in
          let x  =  pi1 p in
