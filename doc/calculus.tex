...
t,t',t'',t_i \in \mathit{SigType} &::= ()~ |~ t + t'~ |~ t \times t'&&\text{(signal types)}\\
\tau,\tau',\tau'',\tau_i \in \mathit{CompType} &::= M(t)&&\text{(computation types)}\\
...
M \in \mathit{Monad} &::= B~ |~ R\\
e,e',e'',e_i \in \mathit{SigExp}&::= x~ e_1~ \dots~ e_n~ |~ \mathbf{let}~ x : t = e~ \mathbf{in}~ e'~ \mathbf{end}\\
...
e_k,e'_k,e_{ki} \in CompExp &::= x~ e_1~ \dots~ e_n~ |~ \mathbf{let}~ x : t = e~ \mathbf{in}~ e_k~ \mathbf{end}\\
  &~~~~|~ \mathbf{return}_M~ e~ |~ \mathbf{bind}~ x : t \leftarrow{} e_k~ \mathbf{in}~ e'_k~ \mathbf{end}~ |~ \mathbf{lift}_T~ e_k\\
  &~~~~|~ \mathbf{signal}_R~ e\\
  &~~~~|~ \mathbf{get}_M~ |~ \mathbf{put}_M~ e\\
  &~~~~|~ \mathbf{rdmem}_M~ e~ |~ \mathbf{wrmem}_M~ e~ e'\\
b \in \mathit{Body}&::= e~ |~ e_k\\
d,d_i \in \mathit{Defn}&::= x~ (x_1 : t_1)~ \dots~ (x_n : t_n) = e\\
P \in \mathit{Prog}&::= \mathbf{let}~ d_1~ \dots~ d_n~ \mathbf{in}~ x~ \mathbf{end}\\
%%--  &~~~~|~ \mathbf{
e,e',e'' \in \mathit{Exp}&::= x~ |~ \lambda{}x_t\rightarrow{}e~ |~ e~ e'&&\text{(lambda calculus)}\\
  &~~~~|~ \mathbf{let}~ x : t = e~ \mathbf{in}~ e'~ \mathbf{end}&&\text{(non-recursive let)}\\
  &~~~~|~ \mathbf{nil}&&\text{(unit)}\\
  &~~~~|~ \mathbf{inl}_{t,t'}~ |~ \mathbf{inr}_{t,t'}~ |~ \mathbf{case}~ e~ \mathbf{of}~ \mathbf{inl}~ x \rightarrow e'~ ; \mathbf{inr}~ y \rightarrow e''~ \mathbf{end}&&\text{(coproduct)}\\
  &~~~~|~ \mathbf{mkpair}_{t,t'}~ |~ \mathbf{fst}_{t,t'}~ |~ \mathbf{snd}_{t,t'}&&\text{(product)}\\
  &~~~~|~ \mathbf{return}_{t,M}~ |~ \mathbf{bind}_{t,t',M}~ |~ \mathbf{lift}_{t,M,T}&&\text{(monad operators)}\\
  &~~~~|~ \mathbf{signal}_{t,t',M}~ |~ \mathbf{unfold}_{t,t',t'',M}&&\text{(reactive prims.)}\\
  &~~~~|~ \mathbf{letrec}~ x_1 : t_1 = e_1~ ; \cdots ; x_n : t_n = e_n~ \mathbf{in}~ e~ \mathbf{end}&&\text{(recursive let)}\\
  &~~~~|~ \mathbf{get}_{t,M}~ |~ \mathbf{put}_{t,M}&&\text{(state prims.)}\\
  &~~~~|~ \mathbf{throw}_{t,t',M}~ |~ \mathbf{catch}_{t,t',M}&&\text{(error prims.)}\\
  &~~~~|~ \mathbf{rdmem}_{t,t',M}~ |~ \mathbf{wrmem}_{t,t',M}&&\text{(memory prims.)}
\end{aligned}
$$
...
See type system below for the all-important restrictions on letrec...
