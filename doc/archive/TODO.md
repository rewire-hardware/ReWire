...
5. Add a translation for the new primitive from RWCrust to the Hyle IR (i.e.,
   define it in terms of Hyle). Extend the `transBuiltin` function in
   `rewire-core/src/ReWire/Crust/ToHyle.hs` with these definitions (there are
   many examples already there). Some primitives, such as Verilog built-in
   operators, you'd instead want to also add new operators to the Hyle
   language (`doc/hyle.md`) and then define those in each back end.
...
7. Run the simple tests with the `-v` RWC flag. Depending on how far RWC gets,
   ...
   Debug: [28] Translating to hyle & HDL.
   Debug: [29] Hyle.
   Debug: Partially evaluating/reducing hyle IR. If this is taking too long, consider disabling with --rtl-opt=0.
   ```
...
8. Dump the IR for the passes leading up to the point of failure. The numbers
   ...
   Hyle. The corresponding IR for a certain pass (or several passes) can be
