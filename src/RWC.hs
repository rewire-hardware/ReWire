...
options =
       ...
       , Option ['d'] ["dump"]            (ReqArg FlagDump        "1,2,...")         "Dump the IR after the given passes (see -v for pass numbers) to files beside the output (e.g., foo.6.eir, foo.8.syn); -v adds the show output in a comment."
       , Option []    ["dump-all"]        (NoArg  FlagDumpAll)                       "Dump the IR after every pass (see -d)."
       , Option []    ["debug-lint"]      (NoArg  FlagDebugLint)                     "Re-lint the IR after every Eidos and Synolon pass to possibly catch compiler bugs."
       , Option []    ["eidos"]           (NoArg  FlagEidos)                         "Dump the Eidos IR after its last pass (.eir) beside the output; see doc/eidos.md."
       , Option []    ["synolon"]         (NoArg  FlagSynolon)                       "Dump the Synolon IR after its last pass (.syn) beside the output; see doc/synolon.md."
       , Option []    ["certify"]         (OptArg FlagCertify     "required|warn")   "Validate the compiled device against the Synolon IR with the formally verified validator (rwv-cstep-validate); compilation fails unless the validator returns VALIDATED (--certify=warn reports the verdict without failing); see doc/certify.md."
