      ...
      where env :: IEnv
            ...
            step sts ins = do
                  unless (null $ devInstances dev) $
                        failAt (ann dev) "cannot evaluate a device with extern instances"
            ...
            stmt (rho, outs, sts) = \ case
                  ...
                  SInstIn an _ _ _ -> failAt an "cannot evaluate a device with extern instances"
...
evalExp env = go
      where go :: HashMap Name BV -> Exp -> m BV
            go rho = \ case
                  ...
                  Var an _ x    -> maybe (failInternal an $ "unbound variable: " <> x) pure $ Map.lookup x rho
                  ...
                  XCall an _ x _ es -> case Map.lookup x (envExterns env) >>= extModel of
                        ...
                        _ -> failAt an $ "cannot evaluate extern " <> x
...
evalOp an op vs = case (op, vs) of
      ...
      _                -> failInternal an $ "ill-formed primitive application: " <> opName op <> " with " <> showt (length vs) <> " arguments"
